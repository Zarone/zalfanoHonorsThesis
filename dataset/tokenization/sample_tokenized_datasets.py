...
Samples tokenized datasets based on MB size, after running
tokenize_datasets.py. Tokenized data should be shuffled and
placed in SHUFFLED_TOKENIZED_DATA. Shuffling for large files
can be done with terashuf:
...
tokenizer_names.remove('.DS_Store')
final_dataset_sizes = dict()
num_dataset_tokens = dict()
...
for tokenizer_name in sorted(tokenizer_names):
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        hf_tokenizer_path, cache_dir='hf_cache'
    )
    ...
    target_sizes = [
        target_size * 1000000 for target_size in target_mb_sizes
    ]  # In bytes.
    ...
    for line_i, line in tqdm(enumerate(infile)):
        ...
        curr_tokens += len(tokenized_sequence)
        if curr_data_size >= target_sizes[target_size_i]:
            ...
            num_dataset_tokens[lang + '_' + quantity_str] = curr_tokens
            target_size_i += 1  # Now wait for the next target size.
            if target_size_i == len(target_sizes):
                reached_max = True
                break
    infile.close()
    print(f"finished reading, got {curr_data_size / 1000000} mb")
    ...
    if tokenizer_quantity_str == '5mb':
        if curr_data_size < 5000000:
            ...
            num_dataset_tokens[lang + '_5mb_dropped'] = curr_tokens
    ...
    if tokenizer_quantity_str == '10mb':
        if curr_data_size < 10000000:
            ...
            num_dataset_tokens[lang + '_10mb_dropped'] = curr_tokens
    ...
    if tokenizer_quantity_str == '100mb':
        if curr_data_size < 100000000:
            ...
            num_dataset_tokens[lang + '_full_but_using_100mb_tokenizer'] = curr_tokens
            continue
    if tokenizer_quantity_str == 'full':
        if curr_data_size < 5000000:
            final_dataset_sizes[lang + '_full_dropped'] = curr_data_size
            num_dataset_tokens[lang + '_full_dropped'] = curr_tokens
            continue  # This language will be dropped entirely, <5mb.
    ...
    if not reached_max:
        ...
        num_dataset_tokens[lang + '_full'] = curr_tokens
...
with open(TRAIN_SIZES_TOKEN_PATH, mode='w', newline='') as f:
    writer = csv.DictWriter(
        f,
        fieldnames=['dataset', 'tokens'],
        delimiter='\t'
    )
    writer.writeheader()
    for key, value in num_dataset_tokens.items():
        writer.writerow({'dataset': key, 'tokens': value})
