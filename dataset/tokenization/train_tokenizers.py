...
from util.constants import TOKENIZER_DATASET_OUTPUT_DIR as TOKENIZER_DATA_DIR
from dataset.tokenization.convert_spm_to_hf_tokenizer import convert

VOCAB_SIZE = 50000
TOKENIZERS_DIR = './dataset/tokenization'
...
for tokenizer_name in sorted(tokenizer_names):
    hf_tokenizer_path = os.path.join(
        TOKENIZERS_DIR, f'monolingual/{tokenizer_name}'
    )
    ...
    convert({
        'input': spm_tokenizer_path + ".model",
        'output_dir': hf_tokenizer_path,
        'keep_accents': True,
        'multiples_of': 2048
    })

    # command = """python \
    # dataset/tokenization/word-acquisition-language-models/scripts/convert_spm_to_hf_tokenizer.py \
    # --input={0}.model \
    # --output_dir={1} \
    # --keep_accents=True \
    # --multiple_of=2048""" \
    # .format(spm_tokenizer_path, hf_tokenizer_path)
