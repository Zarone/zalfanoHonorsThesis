...
def run_tokenize(args):
    ...
    try:
        tokenizer = AutoTokenizer.from_pretrained(args.get('tokenizer'), cache_dir=tokenizer_cache_dir)
    except:
        ...
        tokenizer = AlbertTokenizer.from_pretrained(args.get('tokenizer'))

    max_examples = math.inf if args.get('max_examples') == -1 else args.get('max_examples')
    max_segments = math.inf if args.get('max_segments') == -1 else args.get('max_segments')
    # Cannot input math.inf to the tokenizer, so just use a large number.
    max_seq_len = 999999999 if args.get('max_seq_len') == -1 else args.get('max_seq_len')

    prepend_cls = args.get('prepend_cls', 'True').lower() == "true"
    include_sep = args.get('include_sep', 'True').lower() == "true"
    print(f"Prepending CLS: {prepend_cls}, using SEP: {include_sep}")

    tokenize_file(args.get('input_file'), args.get('output_file'), tokenizer, max_seq_len,
